...
This is a web system for managing servers with GPUs. It allows users to apply for instances with a specified number of GPUs by themselves. Instances of different users are isolated from each other and do not interfere with each other.
...
### Frontend
1. Setup Node 14 and yarn runtime. You can refer to https://codex.so/how-to-install-the-latest-node-js-npm-and-yarn. Note that node version should be 14. 
1. 
```bash
cd client
yarn && yarn install
...
### Backend
It's recommended to create a new conda environment (or a virtual env) for the python backend server:
```bash
conda create -n myenv python=3.8
conda activate myenv
conda install flask flask-cors numpy pymongo
```
...
1. **change the ip in settings.js and app.py to your server ip**.
1. 
```bash
cd server && ./start.sh
cd client && ./start.sh
...
```bash
lxc profile show/edit default # 默认64G一个instance
```
...
```bash
lxc storage info zfs-pool # 查看zfs-pool的使用情况，目前的分区是zfs-pool占用了/dev/sda下的1号分区;
lsblk # 查看所有分区
```
